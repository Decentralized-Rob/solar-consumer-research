...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await auth.supabase
    .from("questions")
    .select("id,state_code,body,status,created_at,updated_at,question_responses(id,body,sources,created_at)")
...
export async function POST(request: NextRequest) {
  ...
  const payload = (await request.json().catch(() => null)) as { stateCode?: unknown; question?: unknown } | null;
  ...
  const { data, error } = await auth.supabase
    .from("questions")
    .insert({ user_id: auth.user.id, state_code: stateCode, body: question })
    .select("id,state_code,body,status,created_at")
    .single();
