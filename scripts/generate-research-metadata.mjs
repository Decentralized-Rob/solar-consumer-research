...
function storyRecord(source, file) {
  ...
  for (const statement of sf.statements) {
    ...
    for (const declaration of statement.declarationList.declarations) {
      ...
      if (!slugProperty || !ts.isStringLiteralLike(slugProperty.initializer)) {
        throw new Error(`${file}: story.slug must be a string literal`);
      }
      return { literal: source.slice(node.getStart(sf), node.getEnd()), slug: slugProperty.initializer.text };
    }
  }
...
for (const file of files) {
  const record = storyRecord(await readFile(resolve(contentDir, file), "utf8"), file);
  const previousFile = slugFiles.get(record.slug);
  if (previousFile) throw new Error(`Duplicate research slug "${record.slug}" in ${previousFile} and ${file}`);
  slugFiles.set(record.slug, file);
  records.push(record.literal);
}
